...
.pc02			
	...
	.import match_name, match_type, skip_mask
...
; kernal_ptr           : if MSB set, copy all bytes to same destination address.
;
...
.ifdef MACHINE_X16
;.importzp kernal_ptr
	bit kernal_ptr        ; MSB=1: stream copy, MSB=0: normal copy
	bpl @5a
	jmp x16_stream_copy
@5a:
...
tmp_swapindex = kernal_ptr ; use meaningful aliases for this tmp space
tmp_done = kernal_ptr+1    ; during bank-aware copy routine
x16_banked_copy:
...
; kernal_ptr           : if MSb set, copy all bytes from same source address.
;
; * c=0: failure; sets errno
;-----------------------------------------------------------------------------
fat32_write:
